...
$(document).ready(function () {
	...
	var calendar = $("#calendar").fullCalendar({
		...
		eventClick: function (calEvent, jsEvent, view) {
			...
			$("#Area_Selector").prop("disabled", false);
			console.log($("#Area_Selector")[0]);
			...
			ZOHO.CRM.API.getRecord({
				...
				if (data.data[0].SJP_Subform.length) {
					// console.log(data.data[0].SJP_Subform[0].Purpose);
					if (
				...
				$("#area_type").val(data.data[0].Area_Type);
	...
	ZOHO.embeddedApp.on("PageLoad", function (data) {
		...
		$(".selectedcustomers .selectpicker").on("hide.bs.select", function () {
			console.log($("#customer_Selector").val());
	...
	ZOHO.CRM.CONNECTION.invoke("widgetconnection", {
		...
		if (data.code == "SUCCESS") {
			...
			if (moreRecords == true) {
				...
				$("#Area_Selector").empty();
				$("#staffAreaSelect").empty();
				$("#Area_Selector").append(insertAllAreas);
