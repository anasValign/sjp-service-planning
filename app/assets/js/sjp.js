...
SJP.MultiCustomers = function (areas, CusFromSubform) {
	...
	console.log("areas", $("#area_type").val());
	console.log("customers");
	if (!$("#area_type").val()) {
		fixForAreaType(areas, userHqId);
	}
	var moreRecords;
		...
		$.each(customers, function (key, val) {
			...
			ZOHO.CRM.API.getRecord({
				...
				if (data.statusText == "nocontent") {
					let resultFromNoContentForCustomers = await noContentForCustomers(customers, CustRecordID)
					my2dArr.push(resultFromNoContentForCustomers);
				}
				...
				if (countravr == lengthOfCus) {
					...
					console.log(testCusArr, testCusArr.length);
					var CusidArray = [];
...
async function noContentForCustomers(customers, CustRecordID) {
	var filteredCustomerNoContent = customers.filter((elem) => elem.Customer_Name.id == CustRecordID);
	if (filteredCustomerNoContent.length > 0) {

		const dataOfNoContent = await ZOHO.CRM.API.getRecord({
			Entity: "Sales_Hierarchy",
			RecordID: filteredCustomerNoContent[0].id,
		})
		console.log("customers nocontent", CustRecordID, filteredCustomerNoContent[0].id, dataOfNoContent);
		let myStrNoContent = {
			Account_Name: dataOfNoContent?.data[0].Customer_Name?.name,
			id: dataOfNoContent?.data[0].Customer_Name?.id,
		};
		return myStrNoContent;
